    ...
    <div class="col-7">
        <div class="card shadow text-black">
            <div class="card-body">
                ...
                <form class="form-material m-t-40" method="post" action="{{route('store_instock_purchase')}}" id="store_purchase">
                    @csrf
    ...
    <div class="col-5">
        ...
        <div class="card shadow text-black">
            ...
            <div class="form-group m-2">
                ...
                <select  name="category" class="p-4 select-cat form-control" id="category" onchange="searchItem(this.value)">
                ...
                <select class="p-4 select form-control" name="item" id="counting_unit_select" >
                    <option></option>
                            ...
                            @foreach ($items as $item)
                                ...
                                <option data-unitname="{{$item->item_name}}"
                                    data-id="{{$item->id}}"
                                    
                                    data-purchaseprice="{{$item->purchase_price}}"
                                    
                                    data-normalprice="{{$item->selling_price}}"
                                    
                                    data-currentqty="{{$item->stock}}"
                                    
                                    value="{{ $item->id }}">
                                    {{ $item->item_code }} &nbsp; &nbsp; - &nbsp; &nbsp;{{ $item->item_name }}&nbsp;&nbsp;{{ $item->stock }} ခု &nbsp; {{$item->purchase_price}} ကျပ်
            ...
            <div class="form-group m-2">
                ...
                <input type="number" id="qty" class="form-control" onkeyup="">
            </div>
...
function searchSubCategory(value){
                ...
                console.log(category_id,item_type);
                        ...
                        success: function(data) {
                            ...
                            $.each(data,function(i,v){
                                    ...
                                    $('#counting_unit_select').append($('<option>').text(v.item_code +"-"+
                                        v.item_name + "  " +  v.stock + "ခု"+"  "+
                                        v.purchase_price + "ကျပ်").attr('value', v.id).data('id',v.id).data('unitname',v.item_name).data('purchaseprice',v.purchase_price).data('normalprice',v.selling_price).data('currentqty',v.stock));
